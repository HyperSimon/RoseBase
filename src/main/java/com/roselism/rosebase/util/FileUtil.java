...
public class FileUtil {
    ...
    public static boolean delDir(String dirPath, boolean delFile) {
        if (delFile) {
            ...
            if (file.isFile()) return file.delete();
                ...
                if (file.listFiles().length == 0) return file.delete();
                else {
        ...
        } else return new File(dirPath).delete();
    }
    ...
    public static void copy(String source, String target, boolean isFolder)
        ...
        if (isFolder) {
            ...
            for (int i = 0; i < file.length; i++) {
                if (source.endsWith(File.separator)) temp = new File(source + file[i]);
                else temp = new File(source + File.separator + file[i]);
                if (temp.isFile()) {
                ...
                if (temp.isDirectory()) copy(source + "/" + file[i], target + "/" + file[i], true);
            }
        } else {
            int byteread = 0;
    ...
    public static boolean move(String source, String target, boolean isFolder)
        ...
        if (isFolder) return delDir(source, true);
        else return delFile(source);
    }
    ...
    public static String getDiskCacheDir(Context context) {
        ...
        else cachePath = context.getCacheDir().getAbsolutePath();
        return cachePath;
    }
}